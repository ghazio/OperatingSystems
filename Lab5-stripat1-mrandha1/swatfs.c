...
static int adder(const char *path, mode_t mode){
  ...
  //if the first block is already full (use the last block allocated)
  else if(inode.blocks>1){
    printf("inside else if\n");
    index_num = ((inode.size % SWATFS_BSIZE) / sizeof(struct dirent)) + 1;
  }
  ...
  int entry_num=(inode.size%SWATFS_BSIZE)/sizeof(struct dirent);
  int block_num=inode.blocks-1;
...
//read in the inode's block
  ...
  strcpy(block[entry_num].name,filename);
  block[entry_num].inum = inum_new;
  free(filename);
  free (temp);
